...
function Resolve-ChangelogEntry {
    param(
        [string]$Type,
        [string]$Title,
        [string]$Description
    ...
    # Claude puede devolver un objeto changelog con "type" pero title/description
    # en blanco (el JSON schema exige los campos presentes, no que sean no vacíos).
    # Tratarlo como "sin changelog" aquí evita que la cadena vacía llegue a
    # Update-ChangelogJson, cuyos parámetros Mandatory la rechazarían.
    $trimmedTitle = if ($Title) { $Title.Trim() } else { "" }
    $trimmedDescription = if ($Description) { $Description.Trim() } else { "" }

    if ($ChangelogTypes -contains $Type -and $trimmedTitle -and $trimmedDescription) {
        return [PSCustomObject]@{ Type = $Type; Title = $trimmedTitle; Description = $trimmedDescription }
    }
    return $null
}
...
"@)
    ...
    [PSCustomObject]@{
        ...
        Changelog  = if ($parsed.changelog) {
            Resolve-ChangelogEntry -Type $parsed.changelog.type -Title $parsed.changelog.title -Description $parsed.changelog.description
        } else { $null }
    }
}
...
function ConvertFrom-DraftText {
    ...
    if ($sections.ContainsKey('CHANGELOG') -and $sections['CHANGELOG']) {
        ...
        $changelog = Resolve-ChangelogEntry -Type $chType -Title $chTitle -Description $chDesc
    }
